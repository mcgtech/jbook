...
def property_new(request):
    return manage_property(request, None)


@login_required
@user_passes_test(back_office_user, 'jbook_login')
def property_edit(request, pk):
    return manage_property(request, pk)
...
def manage_property(request, property_id=None):
    js_dict = {}
    del_request = None
    config = get_form_edit_config(property_id, None, Property, request, 'property_search')

    if property_id is not None:
        del_msg = 'You have successfully deleted the ' + config.class_name + ' ' + str(config.primary_entity)
        del_request = handle_delete_request(request, config, '/property_search', del_msg)
    ...
    if anonymous_user(request.user) == False and property_id is None:
    ...
    return render(request, 'property_edit.html', {'form': primary_entity_form,
                                                           'js_data' : js_data,
                                                           'config' : config,
